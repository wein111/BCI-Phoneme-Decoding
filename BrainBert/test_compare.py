# test_compare.py
# Compare BrainBERT models with different input dimensions (128 vs 256)
...
MODEL_DIR = os.path.join(BASE_DIR, "models", "brainbert")
...
def evaluate_model(model, loader, preproc, num_samples=100):
...
def compare_models():
    ...
    # Model paths
    model_128_path = os.path.join(MODEL_DIR, "best_brainbert_128dim.pt")
    model_256_path = os.path.join(MODEL_DIR, "best_brainbert_256dim.pt")
    
    # Also check for old format
    model_old_path = os.path.join(MODEL_DIR, "best_brainbert.pt")
    
    results = {}
    
    print("\n" + "=" * 60)
    print("BrainBERT Model Comparison: 128-dim vs 256-dim")
    print("=" * 60)
    ...
    if os.path.exists(model_128_path):
        ...
        preproc_128 = BrainBERTPreprocessor(
            smooth_sigma=1.5,
            stack_k=5,
            stack_stride=2,
            subsample_factor=3,
        ...
        print("       Run: python train_brainbert.py with INPUT_DIM=128")
    
    # =========================================================
    # Test 256-dim model
    # =========================================================
    model_256_actual = model_256_path if os.path.exists(model_256_path) else model_old_path
    
    if os.path.exists(model_256_actual):
        print(f"\n>>> Loading 256-dim model from {model_256_actual}")
        ...
        preproc_256 = BrainBERTPreprocessor(
            smooth_sigma=1.5,
            stack_k=5,
            stack_stride=2,
            subsample_factor=3,
        ...
        model_256 = load_model(model_256_actual, input_dim=256)
        results['256-dim'] = evaluate_model(model_256, test_loader_256, preproc_256, num_samples=200)
    else:
        print(f"\n[SKIP] 256-dim model not found at {model_256_path} or {model_old_path}")
        print("       Run: python train_brainbert.py with INPUT_DIM=256")
    ...
    if len(results) == 0:
        ...
        print("  1. Set INPUT_DIM=128 in train_brainbert.py, then run it")
        print("  2. Set INPUT_DIM=256 in train_brainbert.py, then run it")
        return
